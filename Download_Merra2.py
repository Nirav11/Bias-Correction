...
####### INPUTS - CHANGE THESE #########
username = 'Enter you Usernamed' # Username for MERRA download account
password = 'Enter your password' # Password for MERRA download account
years = [2010,2011,2012,2013,2014,2015] # List of years for which data will be downloaed
field_id = 'T2MDEW' # ID of field in MERRA-2 - find ID here: https://gmao.gsfc.nasa.gov/pubs/docs/Bosilovich785.pdf 
field_name = 'Dew Point temperature' # Name of field to be stored with downloaded data (can use any name you like)
...
locs = [('Kyanging', 28.21, 85.56)] # List of locations for which data will be downloaded. Each location is a three-tuple, consisting of name (string), latitude, and longitude floats)
...
for loc, lat, lon in locs:
    ...
    wait = df_hourly
    wait['time'] = wait.index.get_level_values(level=2)
    wait.pop(wait.columns.values[-1])
    wait.columns = [field_name, 'date']
    wait[field_name] = df_hourly[field_name].apply(conversion_function)
    wait.to_csv(field_name + '/' + loc + '_thourly.csv')
    wait = pd.read_csv(field_name + '/' + loc + '_thourly.csv')
    wait = wait.drop(['lat','lon'], axis=1)
    wait.to_csv(field_name + '/' + loc + '_thourly.csv')
    df_daily = wait.groupby('date').agg(aggregator)
    df_daily = df_daily.drop('time', axis=1)
    df_daily['date'] = df_daily.index
    df_daily.pop(df_daily.columns.values[-1])
    df_daily.to_csv(field_name + '/' + loc + '_tdaily.csv')
    df_weekly = pd.read_csv(field_name + '/' + loc + '_tdaily.csv')
